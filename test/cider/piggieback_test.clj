...
(deftest proper-ns-tracking
  ...
  (prn (dorun (nrepl/message *session* {:op "eval" :code "(defn ns-tracking [] (into [] (js/Array 1 2 3)))"})))
...
(deftest defprotocol-no-throw
  ...
  (let [response (-> (nrepl/message *session* {:op "eval" :code "(defprotocol Foo [do-a-thing [this]])"})
                     nrepl/combine-responses)]
    (testing (pr-str response)
      (some-> response :err println)
      (is (:value response)))))
